...
logger = logging.getLogger("A2ALegalNER")
logger.info(f"Logging initialized at {log_level} level")

# --- Model Configuration ---
MODEL_NAME = os.environ.get("A2A_LEGAL_MODEL", "nlpaueb/legal-bert-base-uncased")
AGENT_ID = "a2a_ner_legal"
...
mcp = FastMCP(
    name="LegalNERAgent", 
    version="1.0.0",
    description="Legal Named Entity Recognition agent"
)
...
async def predict(inputs: str, parameters: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Detect legal named entities in text."""
...
async def health_check() -> Dict[str, Any]:
    """Check the health of the Legal NER agent service."""
    request_id = str(uuid.uuid4())[:8]
...
if __name__ == "__main__":
    ...
    port = int(os.environ.get("A2A_LEGAL_PORT", 3004))
    logger.info(f"[{AGENT_ID}] Starting server on port {port}")
    
    # Create a regular FastAPI app
    app = FastAPI(title="Legal NER MCP Server")
