...
logger = logging.getLogger("A2AFinancialNER")
logger.info(f"Logging initialized at {log_level} level")

# --- Model Configuration ---
MODEL_NAME = os.environ.get("A2A_FINANCIAL_MODEL", "ProsusAI/finbert")
AGENT_ID = "a2a_ner_financial"
...
mcp = FastMCP(
    name="FinancialNERAgent", 
    version="1.0.0",
    description="Financial Named Entity Recognition agent"
)
...
async def predict(inputs: str, parameters: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Detect financial named entities in text."""
...
async def health_check() -> Dict[str, Any]:
    """Check the health of the Financial NER agent service."""
    request_id = str(uuid.uuid4())[:8]
...
if __name__ == "__main__":
    ...
    port = int(os.environ.get("A2A_FINANCIAL_PORT", 8007))
    logger.info(f"[{AGENT_ID}] Starting server on port {port}")
    
    # Create a regular FastAPI app
    app = FastAPI(title="Financial NER MCP Server")
